 ...
 * 4. Language switcher (Google Translate, all languages, font-size adjust, RTL)
 */

/* ══════════════════════════════════════════════════════════
   PART 1 — Scroll, Drawer, Active Link
══════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function onScroll() {
    ...
    if (progress) {
      ...
      progress.style.width = (docH > 0 ? (window.scrollY / docH) * 100 : 0).toFixed(2) + '%';
    }
  }
  ...
  if (burger && drawer) {
    ...
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && isOpen) toggleDrawer(false); });
    window.addEventListener('resize', () => { if (isOpen && window.innerWidth > 960) toggleDrawer(false); }, { passive: true });
  }

  /* ── Active link ── */
  let current = window.location.pathname.split('/').pop() || 'home.html';
...
(function () {
  ...
  var LANGUAGES = [
    /* Global (shown first) */
    {c:'en',    n:'English',             nat:'English',           f:'🇺🇸', r:'Global'},
    /* South Asia */
    ...
    {c:'ps',    n:'Pashto',              nat:'پښتو',              f:'🇦🇫', r:'Middle East'},
    /* Europe West */
    ...
    /* Americas */
    {c:'pt-BR', n:'Portuguese (Brazil)', nat:'Português (Brasil)',f:'🇧🇷', r:'Americas'},
  ...
  /* ── Font size map for dense/non-Latin scripts ── */
  var FONT_SIZES = {
    'hi':'105%','mr':'105%','ne':'105%','pa':'105%','gu':'105%',
    'bn':'106%','as':'106%','or':'106%',
    'ta':'106%','te':'106%','kn':'106%','ml':'107%','si':'107%',
    'th':'108%','km':'107%','lo':'107%','my':'107%',
    'zh-CN':'105%','zh-TW':'105%','ja':'105%','ko':'105%',
    'ar':'108%','fa':'108%','ur':'108%','he':'106%','ps':'108%',
    'am':'108%','ti':'107%','ka':'106%','hy':'106%'
  };

  /* ── RTL languages ── */
  var RTL_LANGS = ['ar','fa','ur','he','yi','dv','ps','ug'];
    ...
    regions.forEach(function(region) {
      ...
      LANGUAGES.forEach(function(lang) {
        ...
        btn.innerHTML =
          ...
          '<span class="lang-option-name">' + lang.n +
            '<span class="lang-option-nat"> — ' + lang.nat + '</span></span>' +
  ...
  function selectLang(lang, btn) {
    ...
    /* Font size adjustment for dense scripts */
    document.documentElement.style.fontSize = FONT_SIZES[lang.c] || '100%';

    /* RTL direction */
    document.documentElement.setAttribute('dir',
      RTL_LANGS.indexOf(lang.c) !== -1 ? 'rtl' : 'ltr');
  ...
  /* ── Restore from cookie on page load ── */
  (function() {
    ...
    if (match && match[1] && match[1] !== 'en') {
      ...
      if (found) {
        ...
        document.documentElement.style.fontSize = FONT_SIZES[found.c] || '100%';
        if (RTL_LANGS.indexOf(found.c) !== -1)
